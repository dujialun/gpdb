...
insert into fts_unblock_master values (4);

-- skip FTS probes always
create extension if not exists gp_inject_fault;
select gp_inject_fault('fts_probe', 'reset', dbid) from gp_segment_configuration where master_prober = true;
select gp_inject_fault_infinite('fts_probe', 'skip', dbid) from gp_segment_configuration where master_prober = true;
-- force scan to trigger the fault
0U: select gp_request_fts_probe_scan();
-- verify the failure should be triggered once
select gp_wait_until_triggered_fault('fts_probe', 1, dbid) from gp_segment_configuration where master_prober = true;
...
-1U: select pg_ctl(
                ...
                'start',
                (select port from gp_segment_configuration where content = -1 and preferred_role = 'm'),
                2,
                (select dbid from gp_segment_configuration c where c.role='m' and c.content=-1));
