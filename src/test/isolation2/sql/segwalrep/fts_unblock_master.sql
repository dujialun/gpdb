...
returns text as $$
    ...
    if command in ('stop', 'restart'):
        ...
        opts = '-p %d -\-gp_dbid=%d -i -\-gp_contentid=%d -\-gp_num_contents_in_cluster=3 -E' % (port, dbid, contentid)
...
-1U: select pg_ctl(c.datadir, 'start', c.port, c.content, c.dbid) from gp_segment_configuration c where c.role='m' and c.content=-1;
