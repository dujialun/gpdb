...
static void masterProberConfigUpdate(int16 dbid);
...
/*
 * Master prober is the FTS process running on segment
 */
bool amMasterProber(void)
{
	return (!IS_QUERY_DISPATCHER() && (shmFtsControl->ftsPid == MyProc->pid));
}
...
bool startMasterProber(CdbComponentDatabases *cdbs, bool restart)
{
	...
	if (shmFtsControl->masterProberDBID != masterProberDbid)
	{
		...
		masterProberConfigUpdate(masterProberDbid);
 ...
 */
static
void masterProberConfigUpdate(int16 dbid)
{
...
void FtsLoop()
{
	...
	while (!shutdown_requested)
	{
		...
		has_mirrors = gp_segment_config_has_mirrors(!amMasterProber());
		...
		if (gp_enable_master_autofailover && !amMasterProber() && cdbs->total_entry_dbs == 2)
		{
			/*
